...
public class ArrayListTest {
    ...
    @Test
    @DisplayName("does not swap if Target Value is less than the next")
    public void noSwapIfTargetValueIsLessThanTheNext() {
        addThree();
        newList.swapIfGreaterThanNext(0);
        assertEquals(1, newList.get(0));
    }

    @Test
    @DisplayName("No swap if Target index has no next")
    public void noSwapIfLast() {
        addThree();
        newList.swapIfGreaterThanNext(2);
        assertEquals(5, newList.get(2));
    }

    @Test
    @DisplayName("Swaps values if target value is greater than the next")
